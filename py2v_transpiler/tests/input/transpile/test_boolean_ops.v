...
pub fn test_boolean_or_default() {
    mut name := ''
    mut result := if (name != none && name.len > 0) { Any(name!) } else { Any('Anonymous') }
    println('Default name: ${result}')
    name = 'Alice'
    result = if (name != none && name.len > 0) { Any(name!) } else { Any('Anonymous') }
    println('Actual name: ${result}')
}
// @line: test_boolean_ops.py:65:0
pub fn test_boolean_and_conditional() {
    mut enabled := true
    mut result := if enabled != none { Any('Feature is enabled') } else { Any(enabled!) }
    println('Status: ${result}')
    enabled = false
    result = if enabled != none { Any('Feature is enabled') } else { Any(enabled!) }
    println('Status: ${result}')
}
