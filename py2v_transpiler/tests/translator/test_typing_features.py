...
"""
    ...
    assert "narrowed_val := (val as string)" in v_code or "narrowed_val := string(val)" in v_code
...
"""
    ...
    assert "narrowed_val := (val as int)" in v_code or "narrowed_val := int(val)" in v_code
    # TypeIs narrows the false branch to the remaining type, which is string
    assert "narrowed_else_val := (val as string)" in v_code or "narrowed_else_val := string(val)" in v_code
