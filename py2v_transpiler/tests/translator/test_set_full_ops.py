...
class TestSetFullOps(unittest.TestCase):
    ...
    def test_set_methods_mutation(self):
        self.ti.type_map["s"] = "datatypes.Set[int]"
        self.assertEqual(self.translate_expr("s.add(1)"), "s.add(1)")
        self.assertEqual(self.translate_expr("s.remove(1)"), "py_set_remove(mut s, 1)")
        self.assertEqual(self.translate_expr("s.discard(1)"), "s.elements.delete(1)")
        self.assertEqual(self.translate_expr("s.pop()"), "py_set_pop(mut s)")
        self.assertEqual(self.translate_expr("s.clear()"), "s.elements.clear()")
        self.assertEqual(self.translate_expr("s.copy()"), "s.clone()")

    def test_set_theoretic_methods(self):
        self.ti.type_map["a"] = "datatypes.Set[int]"
    ...
    def test_set_comparisons(self):
        self.ti.type_map["a"] = "datatypes.Set[int]"
        self.ti.type_map["b"] = "datatypes.Set[int]"
    ...
    def test_set_creation_inference(self):
        self.assertEqual(self.translate_expr("set([1, 2])"), "py_set_from_list[datatypes.Set[int]]([1, 2])")
        self.assertEqual(self.translate_expr("set()"), "datatypes.Set[string]{}")
