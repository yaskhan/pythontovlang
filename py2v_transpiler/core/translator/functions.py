...
class FunctionsMixin(TranslatorBase):
    ...
    def _generate_function_for_struct(
        ...
        # 1. Обработка вложенных функций / замыканий
        if is_nested:
            captures = self._find_captured_vars(node)
            capture_str = f"[{', '.join(captures)}] " if captures else ""
            # Используем := для первого объявления в локальной области видимости, иначе =
            decl_op = ":="
        ...
        # 2. Обработка перегрузки операторов
        elif is_method and func_name in ("+", "-", "*", "/", "%", "<", "<=", "==", "!="):
            decl = f"{deprecated_attr}fn {receiver_str}{func_name} ({args_str}) {ret_type} {{"
        
        # 3. Обработка магических методов приведения типов
