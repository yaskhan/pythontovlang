...
class FunctionsMixin(TranslatorBase):
    ...
    def _generate_overload_variants(
        ...
        for sig in self.overloaded_signatures[node.name]:
            ...
            if not is_method and self._is_exported(node.name):
                pub = "pub "
            elif is_method and self.config and getattr(self.config, 'include_all_symbols', False) and not func_name.startswith('_'):
                 pub = "pub "
            ...
            if is_operator:
                ...
                ret_suffix = f" {ret_type}" if ret_type != "void" else ""
                decl = f"{deprecated_attr}{pub}fn {receiver_str}{func_name}{func_generics_str}({args_str}){ret_suffix} {{"
