                ...
                if len(node.value.args) == 2:
                    ...
                    try:
                        if hasattr(ast, 'unparse'):
                             ...
                             mapped_base = self._map_python_type_to_v(base_str, allow_union=True, self_name=self._get_full_self_type())
            ...
            if isinstance(node.value, ast.Call) and isinstance(node.value.func, ast.Name) and node.value.func.id == "TypeVar":
                ...
                for arg in node.value.args[1:]:
                    if isinstance(arg, ast.Name):
                        constraints.append(self._map_python_type_to_v(arg.id, self_name=self._get_full_self_type()))
                    elif isinstance(arg, ast.Constant) and isinstance(arg.value, str):
                        constraints.append(self._map_python_type_to_v(arg.value, self_name=self._get_full_self_type()))
                    ...
                    if kw.arg == "bound":
                        ...
                        # We can use ast.unparse and map
                         try:
                             ...
                             mapped = self._map_python_type_to_v(bound_str, self_name=self._get_full_self_type())
                     ...
                     if hasattr(self, 'type_inference') and lhs in self.type_inference.type_map and isinstance(node.value, ast.Name):
                          ...
                          try:
                              ...
                              if hasattr(ast, 'unparse'):
                                  ...
                                  mapped = self._map_python_type_to_v(rhs_source, allow_union=True, self_name=self._get_full_self_type())
