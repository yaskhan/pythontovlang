...
class ExceptionsMixin(TranslatorBase):
    ...
    def visit_Try(self, node: ast.Try) -> None:
        ...
        self.vexc_depth += 1
        success_var = f"success_{self.unique_id_counter}"
        ...
        if node.handlers:
             exc_var = f"exc_{self.unique_id_counter}"
             self.unique_id_counter += 1
             ...
             # Just try/finally
             self.output.append(f"{self._indent()}exc := vexc.get_curr_exc()")
             self.output.append(f"{self._indent()}vexc.raise(exc.name, exc.msg)")
