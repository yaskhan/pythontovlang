    ...
    def __init__(self, mypy_types):
        ...
        for node, typ in mypy_types.items():
            if hasattr(node, "line") and hasattr(node, "column"):
                # Mypy column vs ast col_offset might have differences,
                # but let's try mapping by line and node type for simplicity if column fails.
                self.node_types[(node.line, type(node).__name__)] = typ
            if hasattr(node, "line") and hasattr(node, "name"):
                self.node_types[(node.line, "name", node.name)] = typ

    def get_type(self, ast_node):
        # Fallback to finding by line and name
        if isinstance(ast_node, ast.Name):
            return self.node_types.get((ast_node.lineno, "name", ast_node.id))
        return None
    ...
    def visit_Call(self, node):
        if isinstance(node.func, ast.Name) and node.func.id == "hasattr":
            ...
            if isinstance(attr_node, ast.Constant) and isinstance(attr_node.value, str):
                ...
                print(f"hasattr on line {node.lineno}: obj {obj_node.id} type = {typ} -> {has_member}")
